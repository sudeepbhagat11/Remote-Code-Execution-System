...
exports.FullProblemDefinitionParser = void 0;
class FullProblemDefinitionParser {
  ...
  const input = require('fs').readFileSync('/dev/problems/${this.problemName
            .toLowerCase()
            .replace(" ", "-")}/tests/inputs/##INPUT_FILE_INDEX##.txt', 'utf8').trim().split('\\n').join(' ').split(' ');
  ${inputReads}
  ${functionCall}
  ${outputWrite}
      `;
    ...
    generateRust() {
        ...
        const inputReads = this.inputFields
            ...
            .join("\n  ");
        const containsVector = this.inputFields.find((field) => field.type.startsWith("list<"));
...
exports.FullProblemDefinitionParser = FullProblemDefinitionParser;
